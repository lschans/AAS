        ...
        execute : function () {
            ...
            net.createServer(function(s) {
                s.on('data', function(d) {
                    worker_index(s.remoteAddress, num_processes, function(num) {
                        var buff = "";
                        var connected = false;
                        var cli = net.createConnection(parseInt(config.global.httpStartPort + num));
                        if (connected) {
                            cli.write(d);
                        } else {
                            buff += d.toString();
            ...
            net.createServer(function(s) {
                s.on('data', function(d) {
                    worker_index(s.remoteAddress, num_processes, function(num) {
                        var buff = "";
                        var connected = false;
                        var cli = net.createConnection(parseInt(config.global.httpsStartPort + num));
                        if (connected) {
                            cli.write(d);
                        } else {
                            buff += d.toString();
...
/*
            server.netHTTP = net.createServer(function(socket) {
                socket.on('data', function (msg) {
                    ...
                    serviceSocket.on('error', function() {
                        console.log("HTTP Socket snafu, my IP: " + socket.remoteAddress + " and PID:" + process.pid);
                    });
            ...
            server.netHTTPS = net.createServer(function(socket) {
                socket.on('data', function (msg) {
                    var serviceSocket = new net.Socket();
                    worker_index(socket.remoteAddress, num_processes, function(num){
                    ...
                    serviceSocket.on('error', function() {
                        console.log("HTTPS Socket snafu, my IP: " + socket.remoteAddress + " and PID:" + process.pid);
                        worker_index(socket.remoteAddress, num_processes, function(num){
                            // Refresh that worker
                            server.cluster.workers[num].kill('SIGUSR2');
                        });
                    });
