...
module.exports = function(server, cluster, net) {
    ...
    return {
        execute : function () {
            ...
            server.net = net.createServer(function(connection) {
                worker_index(connection.remoteAddress, num_processes, function(num){
                    if(config.global.verbose === true) console.log('send ' + ++x + '  -> ' + connection.remoteAddress);
                    server.cluster.workers[num].send('http-sticky-session:connection', connection);
                });
            ...
            server.net = net.createServer(function(connection) {
                worker_index(connection.remoteAddress, num_processes, function(num){
                    if(config.global.verbose === true) console.log(connection.remoteAddress);
                    server.cluster.workers[num].send('https-sticky-session:connection', connection);
                });
