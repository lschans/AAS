    ...
    var serve = {
        serveFile : function(request, response, callback) {
            ...
            */
            ...
            if(fs.existsSync(path + cleanUrl)) {
                ...
                console.log('NORMAL serve ' + process.pid);
            } else if(fs.existsSync('./admin/public' + cleanUrl)){
                // Change path to public
                console.log('PUBLIC serve ' + process.pid);
                path = './admin/public';
            } else {
                // We don't have the file
                console.log('404 serve ' + process.pid);
                response.status = 404;
                response.body = fs.readFileSync('./admin/public/error.html');
            ...
            if(extention == 'html' || extention == 'css' || extention == 'js') {
                ...
                response.body = response.body.toString().replace('@@_SERVER_IP_@@', config.network[0].address);
                response.body = response.body.toString().replace('@@_VERSION_@@', config.global.version);
                response.body = response.body.toString().replace('@@_UPTIME_@@', process.uptime());
                response.body = response.body.toString().replace('@@_DATE_@@', config.git.date);
            }

            if(response.status !== 200) {
                response.body = response.body.toString().replace('@@_ERROR_MESSAGE_@@', config.statusCodes[response.status].message);
                response.body = response.body.toString().replace('@@_STATUS_@@', response.status);
            }
        ...
        write : function (request, response) {
            ...
            response.prepareCookies(request, response, function(){
                ...
                response.end(new Buffer(response.body, 'binary'));
            });
        }
    }
    return serve;
