        ...
        save : function (request, response, callback) {
            Fiber(function() {
                var mongoSync = require("mongo-sync").Server,
                    ...
                    database = mongoServer.db(config.session.database),
                    collection = database.getCollection(config.session.collection),
                    result = collection.update({ "sessionID":sessionID }, { $set: { "sessionID":sessionID, "ip":request.connection.remoteAddress, "dateTime":response.dateCookieString(d), "data":request.session.data}}, {upsert:true});
                callback(request, response);
            }).run();
        },
